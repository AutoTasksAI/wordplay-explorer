...
/** Kid-friendly praise lines; kept short for TTS. "Super" is one option, not the default. */
...
/** Last praise spoken in the previous session (avoid repeating across back-to-back sessions). */
...
/** One praise line per round; no two consecutive lines use the same wording. */
export function buildSessionPraises(roundCount: number): string[] {
  const praises: string[] = [];
  let previous: string | null =
    roundCount > 0 ? lastSessionPraise : null;
  ...
  if (praises.length > 0) {
    lastSessionPraise = praises[praises.length - 1];
  }
  return praises;
}

/** Spoken after a correct tap, e.g. "Fantastic! ball!" */
export function formatPraiseUtterance(praise: string, praiseWord: string): string {
  return `${praise} ${praiseWord}!`;
}

/** @deprecated Prefer buildSessionPraises for a full session. */
